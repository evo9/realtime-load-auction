...
### Не оставляй за собой мусор
Задача считается законченной, только если после неё не осталось ничего, что ты запустил или создал.

- **Проверяй одноразовыми командами.** `pnpm -C apps/api build` / `test` / `lint`, разовый `node dist/main` — они завершаются сами. Watcher (`pnpm dev`, `dev:api`, `nest start --watch`, `next dev`) поднимать только если задача про сам watch-режим; для «проверить, что собралось и отвечает» он не нужен.
- **Запустил долгоживущее — ты владеешь его жизненным циклом.** Забирай PID при старте и гаси в той же задаче, до отчёта в чате. Не «потом», не «в следующей итерации».
- **Watcher гасится группой, а не по PID.** `nest start --watch` и `next dev` порождают детей; убийство родителя оставляет сирот, которые держат порт. Гаси `pkill -f "nest start"` / по группе процессов.
- **Перед отчётом убедись, что чисто:** `pgrep -f "nest start|next dev"` и `lsof -i :3000` (api) / `:3001` (web) — пусто. Не пусто — ты не закончил.
- **Временные файлы — туда же.** Пробники, скрипты-однодневки, `probe/`-папки удаляются в той же задаче. В репозитории не остаётся ничего, чего не просил пользователь.
- **Исключение — инфра из `make up`.** Postgres/RabbitMQ/Redis в docker-compose подняты пользователем и живут между задачами: не трогай их и не гаси. Правило про мусор — о том, что запустил **ты**, а не о его окружении.
