...
- Сейчас в `src/` — дефолтный скаффолд (`app.controller/service/module`, `main.ts`). Целевая структура ниже ещё не создана — она появляется по задачам M1–M4.
...
- Алиас `@src/*` (`tsconfig-paths`) — ввести при наполнении (сейчас в tsconfig не настроен).
