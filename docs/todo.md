...
- [x] `integration-test` — через `act` не удалось проверить до конца (Docker-outside-of-Docker сетевая ловушка — testcontainers внутри act-контейнера не резолвит `localhost` до портов на хосте, ограничение симуляции `act`, не GitHub Actions). Гипотеза подтвердилась: на реальном GitHub Actions (после пуша) `integration-test` прошёл полностью зелёным — все сьюты `PASS`, включая retry-dlx/outbox/dedup/dlq-inspector/settlement-step.consumer (видимые в логе WARN/ERROR — намеренная инъекция сбоев самими тестами, ожидаемо)
...
## Pipeline
- [x] Запись в docs/worklog.md + галочка в docs/tasks/INDEX.md (закрывает M6)
